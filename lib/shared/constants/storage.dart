class StorageConstants {
  // 账号相关
  static final String number = 'number';
  static final String passwd = 'passwd';
  static final String cookie = 'cookie';
  static final String role = 'role'; // 是否为研究生

  // 个人信息
  static final String name = 'name'; // 姓名
  static final String execution = 'execution'; // 书院
  static final String institutes = 'institutes'; // 学院
  static final String subject = 'subject'; // 专业
  static final String dorm = 'dorm'; // 宿舍，如为纯数字即为电费账号
  static final String currentSemester = 'currentSemester'; // 当前学期编码
  static final String currentStartDay = 'currentStartDay'; // 当前学期编码




}